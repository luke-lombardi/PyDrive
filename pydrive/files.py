...
MIME_TYPE_TO_BOM = {
    "application/vnd.google-apps.document": {"text/plain": u"\ufeff".encode("utf8")}
}
...
class GoogleDriveFileList(ApiResourceList):
    ...
    def _GetList(self):
        ...
        self.metadata = (
            self.auth.service.files().list(**dict(self)).execute(http=self.http)
        )
...
class GoogleDriveFile(ApiAttributeMixin, ApiResource):
    ...
    def FetchContent(self, mimetype=None, remove_bom=False):
        ...
        if download_url:
            ...
            self.content = io.BytesIO(self._DownloadFromUrl(export_links.get(mimetype)))
            self.dirty["content"] = False

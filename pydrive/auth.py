...
class GoogleAuth(ApiAttributeMixin, object):
    ...
    def ServiceAuth(self):
        ...
        self.credentials = ServiceAccountCredentials.from_p12_keyfile(
            service_account_email=service_email, filename=file_path, scopes=scopes
        )
        if user_email:
    ...
    def LoadClientConfigFile(self, client_config_file=None):
        ...
        if not client_type in (clientsecrets.TYPE_WEB, clientsecrets.TYPE_INSTALLED):
    ...
    def LoadServiceConfigSettings(self):
        ...
        for config in self.SERVICE_CONFIGS_LIST:
            try:
                self.client_config[config] = self.settings["service_config"][config]
            except KeyError:
    ...
    def GetFlow(self):
        ...
        :raises: InvalidConfigError
        """
        if not all(config in self.client_config for config in self.CLIENT_CONFIGS_LIST):
    ...
    def Refresh(self):
        ...
        if self.credentials.refresh_token is None:
            raise RefreshError(
                "No refresh_token found." "Please set access_type of OAuth to offline."
            )
